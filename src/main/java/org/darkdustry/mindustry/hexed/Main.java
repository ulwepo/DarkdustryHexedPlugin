				...
				if (active()) {
					...
					for (Player player : Groups.player) {
						if (player.team() != Team.derelict && player.team().cores().isEmpty()) {
							...
							Groups.player.each(
								p -> bundled(p, "server.player-lost", player.coloredName())
							);
							Call.infoMessage(
								player.con,
								Bundle.format("server.you-lost", findLocale(player))
							);
					...
					if (interval.get(timerBoard, leaderboardTime)) {
						Groups.player.each(
							player -> Call.infoToast(player.con, getLeaderboard(player), 12.5f)
						);
					}
					...
					if (interval.get(timerWinCheck, 60 * 2)) {
						...
						if (
							!players.isEmpty() &&
							data.getControlled(players.first()).size >= winCondition &&
							players.size > 1 &&
			...
			event -> {
				if (active() && event.tile.block() instanceof CoreBlock) {
					...
					if (hex != null) {
						...
						if (
							players.size > 2 &&
							players.count(p -> p.team() != Team.derelict) == 1 &&
		...
		Events.on(
			...
			event -> {
				...
				if (hex != null) {
					...
					Call.infoMessage(
						event.player.con,
						Bundle.format("server.no-empty-hex", findLocale(event.player))
					);
						...
						new Subscriber<Document>() {
							...
							public void onNext(Document t) {
								if (!Objects.isNull(t)) players
									...
									.append("\n"); else players.append(
									Bundle.format("commands.lb.none", findLocale(player))
								);
							}
		...
		handler.<Player>register(
			...
			(args, player) -> {
				...
				if (hex != null) {
					...
					status
						.append(Bundle.format("commands.hexstatus.hex", findLocale(player)))
						.append(hex.id)
						.append("[]\n");
					status
						.append(Bundle.format("commands.hexstatus.owner", findLocale(player)))
						.append(
							hex.controller != null && data.getPlayer(hex.controller) != null
								? data.getPlayer(hex.controller).coloredName()
								: Bundle.format("commands.hexstatus.owner.none", findLocale(player))
						)
						.append("\n");
					for (Teams.TeamData data : state.teams.getActive()) {
						if (
							...
							status
								.append("[white]|> [accent]")
								.append(this.data.getPlayer(data.team).coloredName())
		...
		if (!players.isEmpty()) {
			...
			for (Player player : Groups.player) {
				...
				if (!dominated) endGameMessage +=
					Bundle.format("final-score", findLocale(player), builder.toString());
	...
	public void updateText(Player player) {
		...
		if (team.location.controller == null) {
			if (team.progressPercent > 0) {
				message
					.append(Bundle.format("capture-progress", findLocale(player)))
			...
			message
				...
				.append("]")
				.append(Bundle.format("hex-captured-by-player", findLocale(player)))
