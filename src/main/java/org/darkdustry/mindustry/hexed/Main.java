package org.darkdustry.mindustry.hexed;
...
import org.darkdustry.mindustry.hexed.HexData.HexCaptureEvent;
import org.darkdustry.mindustry.hexed.HexData.HexMoveEvent;
import org.darkdustry.mindustry.hexed.HexData.HexTeam;
import org.darkdustry.mindustry.hexed.HexData.ProgressIncreaseEvent;
import org.darkdustry.mindustry.hexed.comp.Bundle;
import org.darkdustry.mindustry.hexed.comp.ConfigurationManager;
import org.darkdustry.mindustry.hexed.comp.NoPauseRules;
import org.darkdustry.mindustry.hexed.models.UserStatistics;
...
public class Main extends Plugin {
    ...
    public void init() {
                ...
                if (active()) {
                    ...
                    for (Player player : Groups.player) {
                        ...
                        if (player.team() == Team.derelict) {
                            ...
                            data.getControlled(player).size == data.hexes().size
                        ) {
                            endGame();
                            break;
                        }
                    ...
                    if (interval.get(timerWinCheck, 60 * 2)) {
                        ...
                        if (
                            !players.isEmpty() &&
                            data.getControlled(players.first()).size >=
                            winCondition &&
                            players.size > 1 &&
                            data.getControlled(players.get(1)).size <= 1
                        ) {
                            endGame();
                        }
                    }
            ...
            event -> {
                if (active() && event.tile.block() instanceof CoreBlock) {
                    ...
                    if (hex != null) {
                        ...
                        if (
                            players.size > 2 &&
                            players.count(p -> p.team() != Team.derelict) ==
                            1 &&
                            data.getControlled(players.first()).size > 1
                        ) endGame();
                    }
                }
            }
        ...
        Events.on(
            ...
            event -> {
                if (active() && event.player.team() != Team.derelict) {
                    ...
                    Timer.schedule(
                        () -> {
                            int count = Groups.player.count(
                                p -> p.team() == event.player.team()
                            );
                            if (count > 0) return;
        ...
        Events.on(
            ...
            event -> {
                ...
                copy.shuffle();
                Hex hex = copy.find(
                    h -> h.controller == null && h.spawnTime.get()
                );
        ...
        Events.on(
            EventType.WorldLoadEvent.class,
            event ->
                Time.runTask(
                    5f,
                    () -> {
        ...
        Events.on(
            ProgressIncreaseEvent.class,
            event -> updateText(event.player)
        );
        ...
        netServer.assigner =
            (player, players) -> {
                ...
                if (active()) {
                    if (
                        teamTimers.containsKey(player.uuid())
                    ) return teamTimers.get(player.uuid());
                    ...
                    Call.infoMessage(
                        player.con,
                        Bundle.format("server.no-empty-hex", findLocale(player))
                    );
        ...
        netServer.chatFormatter =
            (player, message) -> {
                ...
                if (active()) {
                    ...
                    UserStatistics.find(
                        new BasicDBObject(Map.of("UUID", player.uuid())),
                        userStatistic -> {
                            userStatistic.name = player.name;
                            userStatistic.save();

                            wins.set(userStatistic.wins);
                        }
                    );
    ...
    public void registerClientCommands(CommandHandler handler) {
        handler.<Player>register(
            "lb",
            "Показать текущих лучших игроков сервера.",
            (args, player) -> {
                ...
                UserStatistics
                    .getSourceCollection()
                    .find()
                    .sort(new BasicDBObject("wins", new BsonInt32(-1)))
                    .limit(10)
                    .subscribe(
                        new Subscriber<Document>() {
                            ...
                            public void onNext(Document t) {
                                if (!Objects.isNull(t)) players
                                    .append("[accent]")
                                    .append(cycle[0]++)
                                    .append(". ")
                                    .append(t.getString("name"))
                                    .append("[accent]: [cyan]")
                                    .append(t.getInteger("wins"))
                                    .append("\n"); else players.append(
                                    Bundle.format(
                                        "commands.lb.none",
                                        findLocale(player)
                            ...
                            public void onError(Throwable t) {
                                if (!Objects.isNull(t)) t.printStackTrace();
                            }
        ...
        handler.<Player>register(
            "spectator",
            "Режим наблюдателя. Уничтожает твою базу",
            (args, player) -> {
                if (player.team() == Team.derelict) {
        ...
        handler.<Player>register(
            "captured",
            "Узнать количество захваченных хексов.",
            (args, player) -> {
                if (player.team() == Team.derelict) {
                ...
                bundled(
                    player,
                    "commands.captured.hexes",
                    data.getControlled(player).size
                );
            }
        );
        ...
        handler.<Player>register(
            ...
            (args, player) -> player.sendMessage(getLeaderboard(player))
        );

        handler.<Player>register(
            "hexstatus",
            "Узнать статус хекса на своем местоположении.",
            (args, player) -> {
                ...
                if (hex != null) {
                    ...
                    status
                        .append(
                            Bundle.format(
                                "commands.hexstatus.owner",
                                findLocale(player)
                            )
                        )
                        .append(
                            hex.controller != null &&
                                data.getPlayer(hex.controller) != null
                                ? data.getPlayer(hex.controller).coloredName()
                                : Bundle.format(
                                    "commands.hexstatus.owner.none",
                                    findLocale(player)
                                )
                        )
                        .append("\n");
                    for (Teams.TeamData data : state.teams.getActive()) {
                        if (
                            hex.getProgressPercent(data.team) > 0 &&
                            hex.getProgressPercent(data.team) <= 100
                        ) {
                            status
                                .append("[white]|> [accent]")
                                .append(
                                    this.data.getPlayer(data.team).coloredName()
                                )
                                .append("[lightgray]: [accent]")
                                .append(
                                    Bundle.format(
                                        "commands.hexstatus.captured",
                                        findLocale(player),
                                        (int) hex.getProgressPercent(data.team)
                                    )
                                )
                                .append("\n");
                        }
    ...
    public void registerServerCommands(CommandHandler handler) {
        ...
        handler.register(
            "hexed",
            "[mode/list]",
            "Запустить сервер в режиме Хексов.",
            args -> {
                ...
                if (args.length > 0) {
                    try {
                        ...
                        Log.err(
                            "Неверное название режима. Будет выбран случайный режим."
                        );
                    }
                }
                mode =
                    custom == null
                        ? Structs.random(HexedGenerator.Mode.values())
                        : custom;
    ...
    public void endGame() {
        ...
        for (int i = 0; i < players.size && i < 4; i++) {
            if (data.getControlled(players.get(i)).size > 1) {
                builder
                    ...
                    .append(i + 1)
                    .append(".[accent] ")
                    .append(players.get(i).name)
                    .append("[lightgray] (x")
                    .append(data.getControlled(players.get(i)).size)
                    .append(")[]\n");
            }
        }
        ...
        if (!players.isEmpty()) {
            ...
            for (Player player : Groups.player) {
                String endGameMessage = Bundle.format(
                    "round-over",
                    findLocale(player)
                );

                if (player == players.first()) endGameMessage +=
                    Bundle.format(
                        "you-won",
                        findLocale(player),
                        data.getControlled(players.first()).size
                    ...
                    Bundle.format(
                        "player-won",
                        findLocale(player),
                        players.first().coloredName(),
                        data.getControlled(players.first()).size
                    );

                if (!dominated) endGameMessage +=
                    Bundle.format(
                        "final-score",
                        findLocale(player),
                        builder.toString()
    ...
    public void updateText(Player player) {
        ...
        StringBuilder message = new StringBuilder(
            Bundle.format("hex", findLocale(player)) + team.location.id + "\n"
        );
        ...
        if (team.location.controller == null) {
            if (team.progressPercent > 0) {
                message
                    .append(
                        Bundle.format("capture-progress", findLocale(player))
                    )
                    .append((int) (team.progressPercent))
                    .append("%");
            } else {
            ...
            team.location != null &&
            team.location.controller != null &&
            data.getPlayer(team.location.controller) != null
        ) {
            message
                .append("[#")
                .append(team.location.controller.color)
                .append("]")
                .append(
                    Bundle.format("hex-captured-by-player", findLocale(player))
                )
                .append(data.getPlayer(team.location.controller).coloredName());
        } else {
    ...
    public String getLeaderboard(Player p) {
        StringBuilder builder = new StringBuilder();
        builder.append(
            Bundle.format("leaderboard.header", findLocale(p), lastMin)
        );
        int count = 0;
    ...
    public void killTiles(Team team) {
        ...
        Groups.unit.each(
            u -> u.team == team,
            unit -> Time.run(Mathf.random(360), unit::kill)
        );
    ...
    public void loadout(Player player, int x, int y) {
        ...
        if (coreTile == null) throw new IllegalArgumentException(
            "Schematic has no core tile. Exiting."
        );
    ...
    private static Locale findLocale(Player player) {
        Locale locale = Structs.find(
            ...
            l ->
                l.toString().equals(player.locale) ||
