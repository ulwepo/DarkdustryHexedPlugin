...
import static org.darkdustry.mindustry.hexed.Main.mode;
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				...
				int elev = Mathf.clamp(
					(int) (
						((Simplex.noise2d(s2, 12, 0.6, 1.0 / 700, x, y) - 0.5) * 10 + 0.15f) *
				...
				if (
					floor == Blocks.tar || floor == Blocks.slag || floor == Blocks.cryofluid
				) ore = Blocks.air;
						...
						Bresenham2.line(
							...
							(cx, cy) ->
								Geometry.circle(
									...
									height,
									3,
									(c2x, c2y) -> tiles.getn(c2x, c2y).setBlock(Blocks.air)
								)
						);
					}
				}
			...
			for (int y = 0; y < height; y++) {
				...
				if (wall == Blocks.air) {
					if (Mathf.chance(0.03)) {
						if (floor == Blocks.sand) wall = Blocks.sandBoulder; else if (
							floor == Blocks.stone
						) wall = Blocks.boulder; else if (floor == Blocks.shale) wall =
							Blocks.shaleBoulder; else if (floor == Blocks.darksand) wall =
							Blocks.boulder; else if (floor == Blocks.moss) wall =
							Blocks.sporeCluster; else if (floor == Blocks.ice) wall =
							Blocks.snowBoulder; else if (floor == Blocks.snow) wall =
							Blocks.snowBoulder;
					}
				}
				tile.setBlock(wall);
	...
	public IntSeq getHex() {
		...
		for (int x = 0; x < width / Hex.spacing - 2; x++) {
			for (int y = 0; y < height / (h / 2) - 2; y++) {
				int cx = (int) (x * Hex.spacing * 1.5 + (y % 2) * Hex.spacing * 3.0 / 4) +
				Hex.spacing /
				2;
